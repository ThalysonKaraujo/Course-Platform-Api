...
import com.thalyson.digitalcourses.course_platform_backend.dto.DadosDetalhamentoUserr;
...
public class UserController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Detalhes do perfil retornados com sucesso",
                    content = @Content(schema = @Schema(implementation = DadosDetalhamentoUserr.class))),
            @ApiResponse(responseCode = "401", description = "Não autenticado")
    })
    public ResponseEntity<DadosDetalhamentoUserr> getMyProfile() {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        UserJPA authenticatedUser = (UserJPA) authentication.getPrincipal();
        return ResponseEntity.ok(new DadosDetalhamentoUserr(authenticatedUser));
    }
